    ...
    <div className="space-y-6">
      <PageHeader
        ...
        action={
          <Button onClick={() => setEditing({
            title: "", description: "", badge_color: "#e63946",
            is_pinned: false, announcement_date: new Date().toISOString().slice(0, 10),
          })}>
        ...
        {items.map((a) => (
          <Card key={a.id}>
            <div className="flex items-start gap-3">
              <div className="w-1.5 self-stretch rounded-full" style={{ background: a.badge_color }} />
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  {a.is_pinned && <span className="text-xs bg-primary text-primary-foreground px-2 py-0.5 rounded-full">📌 Pinned</span>}
              ...
              <div className="flex">
                <button onClick={() => togglePin(a)} className="p-2 rounded-md hover:bg-muted text-sm" title="Toggle pin">
                  📌
                </button>
                <IconButton icon={Pencil} label="Edit" onClick={() => setEditing(a)} />
                <IconButton icon={Trash2} label="Delete" variant="danger" onClick={() => setDeleting(a)} />
              </div>
            </div>
          </Card>
        ))}
        {items.length === 0 && <Card><p className="text-muted-foreground text-sm">No announcements yet.</p></Card>}
      </div>
      ...
      <Modal
        ...
        footer={<>
          ...
          <Button onClick={() => (document.getElementById("ann-form") as HTMLFormElement)?.requestSubmit()}>
      ...
      <ConfirmDelete open={!!deleting} onClose={() => setDeleting(null)} onConfirm={del} label={deleting?.title} />
    </div>
  );
}

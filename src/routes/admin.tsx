    ...
    >
      <div className="w-full max-w-md bg-card rounded-2xl shadow-xl border p-8">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold" style={{ color: "var(--navy)" }}>Admin Login</h1>
          <p className="text-sm text-muted-foreground mt-1">Sign in to manage Payitragam</p>
        </div>
        <form onSubmit={onSubmit} className="space-y-4">
          <Input label="Email" type="email" value={email} onChange={setEmail} placeholder="admin@gmail.com" />
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-background">
      <div className="max-w-2xl w-full bg-card rounded-2xl border p-8 shadow-lg">
        ...
        <div className="space-y-4 text-sm">
          <Step n={1}>
            Go to your Supabase dashboard → <b>Project Settings → API</b> → copy the <b>Project URL</b>.
          </Step>
          <Step n={2}>
            Open <code className="bg-muted px-1.5 py-0.5 rounded">src/lib/supabase.ts</code> and replace
            the <code className="bg-muted px-1.5 py-0.5 rounded">YOUR-PROJECT-REF</code> placeholder.
          </Step>
          <Step n={3}>
            Run the SQL setup script (<code className="bg-muted px-1.5 py-0.5 rounded">SUPABASE_SETUP.sql</code> in
            project root) in your Supabase <b>SQL Editor</b>.
          </Step>
          <Step n={4}>
          ...
          <div className="bg-destructive/10 text-destructive text-xs p-3 rounded-lg mt-4">
            <b>Security warning:</b> The Supabase secret key you shared in chat is exposed.
            Rotate it immediately in Supabase → Project Settings → API → "Reset service_role key".
            Only the publishable (anon) key belongs in client code.
          </div>
        </div>
      </div>
    </div>

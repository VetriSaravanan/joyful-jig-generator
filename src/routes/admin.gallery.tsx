...
import {
  Card, Field, Button, PageHeader, Modal, ConfirmDelete, IconButton,
  Pencil, Trash2, Plus, Upload, Save,
...
type Img = { id: string; image_url: string; caption: string | null; category_id: string | null; order_index: number };
...
function GalleryPage() {
  ...
  async function addCategory() {
    ...
    const { error } = await supabase.from("gallery_categories")
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-bold ${
              activeCat === "all" ? "bg-primary text-primary-foreground" : "bg-muted"
            }`}
          >
            All ({imgs.length})
          </button>
          {cats.filter((c) => c.slug !== "all").map((c) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold inline-flex items-center gap-2 ${
                  activeCat === c.slug ? "bg-primary text-primary-foreground" : "bg-muted"
                }`}
              >
                {c.name} ({count})
                <button
                  onClick={(e) => { e.stopPropagation(); delCategory(c); }}
      ...
      {filtered.length === 0 ? (
        <Card><p className="text-muted-foreground text-sm">No images in this category yet.</p></Card>
      ) : (
            ...
            return (
              <div key={img.id} className="bg-card rounded-xl border overflow-hidden group">
                ...
                <div className="p-3">
                  ...
                  {img.caption && <div className="text-sm font-medium truncate mt-1">{img.caption}</div>}
                  <div className="flex gap-1 mt-2">
                    <IconButton icon={Pencil} label="Edit" onClick={() => setEditing(img)} />
                    <IconButton icon={Trash2} label="Delete" variant="danger" onClick={() => setDeleting(img)} />
                  </div>
                </div>
              </div>
            );
      ...
      <Modal
        ...
        footer={<>
          ...
          <Button onClick={saveImg}><Save className="w-4 h-4" /> Save</Button>
        </>}
      >
        {editing && (
          <div className="space-y-4">
            <img src={editing.image_url} alt="" className="w-full max-h-64 object-contain rounded-lg border" />
      ...
      <ConfirmDelete open={!!deleting} onClose={() => setDeleting(null)} onConfirm={delImg} label="this image" />
    </div>
  );
}

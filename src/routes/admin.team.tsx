...
type Member = { id: string; name: string; role: string; photo_url: string | null; order_index: number };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={<Button onClick={() => setEditing({ name: "", role: "", photo_url: null, order_index: items.length })}><Plus className="w-4 h-4" /> Add Member</Button>}
      />

      {items.length === 0 ? (
        <Card><p className="text-muted-foreground text-sm">No team members yet.</p></Card>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((m) => (
            <Card key={m.id} className="text-center">
              {m.photo_url ? (
                <img src={m.photo_url} alt={m.name} className="w-24 h-24 rounded-full mx-auto object-cover border-2 border-primary" />
              ) : (
                <div className="w-24 h-24 rounded-full mx-auto bg-muted flex items-center justify-center text-2xl">👤</div>
              )}
              <h3 className="font-semibold mt-3">{m.name}</h3>
              <p className="text-sm text-muted-foreground">{m.role}</p>
              <div className="flex justify-center gap-1 mt-3">
                <IconButton icon={Pencil} label="Edit" onClick={() => setEditing(m)} />
                <IconButton icon={Trash2} label="Delete" variant="danger" onClick={() => setDeleting(m)} />
              </div>
            </Card>
          ))}
        </div>
      ...
      <Modal
        ...
        footer={<>
          ...
          <Button type="submit" onClick={() => { const f = document.getElementById("team-form") as HTMLFormElement | null; f?.requestSubmit(); }}>

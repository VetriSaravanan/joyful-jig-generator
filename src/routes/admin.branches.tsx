...
type Branch = { id: string; branch_name: string; address: string; map_embed_url: string | null; phone: string; order_index: number };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={<Button onClick={() => setEditing({ branch_name: "", address: "", phone: "", order_index: items.length })}><Plus className="w-4 h-4" /> Add Branch</Button>}
      />
          ...
          <Card key={b.id}>
            <div className="flex justify-between items-start gap-2">
              ...
              <div className="flex">
                ...
                <IconButton icon={Trash2} label="Delete" variant="danger" onClick={() => setDeleting(b)} />
              </div>
            </div>
      ...
      <Modal
        ...
        footer={<>
          ...
          <Button onClick={() => (document.getElementById("branch-form") as HTMLFormElement)?.requestSubmit()}>
      ...
      <ConfirmDelete open={!!deleting} onClose={() => setDeleting(null)} onConfirm={del} label={deleting?.branch_name} />
    </div>
  );
}

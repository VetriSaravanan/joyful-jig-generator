...
export function Card({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <div className={`bg-card rounded-xl border p-5 lg:p-6 ${className}`}>{children}</div>
  );
}

export function Field({
  label, value, onChange, type = "text", placeholder, required, textarea, rows = 3,
}: {
  label: string;
  value: string;
  ...
  return (
    <label className="block">
      ...
      {textarea ? (
        <textarea rows={rows} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} required={required} className={cls + " resize-y"} />
      ) : (
...
export function IconButton({
  onClick, icon: Icon, label, variant = "ghost",
}: {
  onClick: () => void;
  icon: typeof Pencil;
  label: string;
  ...
  return (
    <button
      ...
      className={`p-2 rounded-md transition ${
        variant === "danger" ? "text-destructive hover:bg-destructive/10" : "hover:bg-muted"
      }`}
    >
...
export function ConfirmDelete({
  open, onClose, onConfirm, label = "this item",
}: {
  open: boolean;
  onClose: () => void;
  onConfirm: () => void;
  ...
  return (
    ...
    >
      <p>Are you sure you want to delete <b>{label}</b>? This action cannot be undone.</p>
    </Modal>
  );
}
